...
namespace Watermelon
{
    public class SkinsController : MonoBehaviour, ISkinsProvider
    {
        ...
        private Dictionary<AbstractSkinsProvider, ISkinData> selectedSkins;
        ...
        public void Init()
        {
            ...
            selectedSkins = new Dictionary<AbstractSkinsProvider, ISkinData>();

            for (int i = 0; i < handler.ProvidersCount; i++)
            {
                AbstractSkinsProvider provider = handler.GetSkinsProvider(i);
        ...
        private void InitProvider(AbstractSkinsProvider provider)
        {
            provider.Init();
        ...
        public ISkinData GetSelectedSkin<T>() where T : AbstractSkinsProvider
        {
            AbstractSkinsProvider provider = GetProvider<T>();

            return GetSelectedSkin(provider);
        }

        private ISkinData GetSelectedSkin(AbstractSkinsProvider provider)
        {
        ...
        private ISkinData UnlockAndSelect<T>() where T : AbstractSkinsProvider
        {
            AbstractSkinsProvider provider = GetProvider<T>();
        ...
        private ISkinData UnlockAndSelectDefaultSkin(AbstractSkinsProvider provider)
        {
        ...
        {
            AbstractSkinsProvider provider = data.SkinsProvider;
        ...
        private AbstractSkinsProvider GetProvider<T>() where T : AbstractSkinsProvider
        {
            for (int i = 0; i < handler.ProvidersCount; i++)
            {
                AbstractSkinsProvider provider = handler.GetSkinsProvider(i);
