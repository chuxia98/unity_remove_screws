    ...
    public class SkinPickerWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (handlerData.Tabs.IsInRange(tabIndex))
            {
                ...
                AbstractSkinsProvider provider = selectedTab.SkinsProvider;
        ...
        {
            public SkinsHandler SkinsHandler { get; private set; }
            ...
            public HandlerData(System.Type filteredType)
            {
                ...
                {
                    Debug.LogError("Skins Handler is missing!");
                ...
                {
                    Tabs = new Tab[1];
                    Tabs[0] = new Tab(SkinsHandler.GetSkinsProvider(filteredType));
                }
                else
                {
                    AbstractSkinsProvider[] skinProviders = SkinsHandler.SkinsProviders;
                    Tabs = new Tab[skinProviders.Length];
                    TabNames = new string[skinProviders.Length];

                    for (int i = 0; i < skinProviders.Length; i++)
                    {
            ...
            public class Tab
            {
                ...
                public AbstractSkinsProvider SkinsProvider { get; private set; }

                private FieldInfo previewFieldInfo;

                public Tab(AbstractSkinsProvider skinsProvider)
                {
                    SkinsProvider = skinsProvider;

                    Name = GetProviderName();

                    previewFieldInfo = SkinsProvider.Skins.GetType().GetElementType().GetFields(ReflectionUtils.FLAGS_INSTANCE).First(x => x.GetCustomAttribute<SkinPreviewAttribute>() != null);
                }
